...
describe('données de carte', () => {
  ...
  it('conserve les coordonnées d’une sous-station créée par projet', () => {
    ...
    const effective = getEffectiveSubstations([], [{ id: 'proj-map', status: 'validé', year: 2026, effects: [effect] }], 2026);
  ...
  it('positionne une demande par clic carte sans toucher aux autres dossiers', () => {
    ...
    expect(updated.connectionRequests.find(r => r.id === 'req-map').customer.site.coordinates)
      .toEqual({ lat: 50.62, lng: 5.58, source: 'map_click' });
    expect(updated.connectionRequests.find(r => r.id === 'req-other').customer.site.coordinates)
  ...
  it('liste toutes les demandes avec impact capacité actif, indépendamment de la file', () => {
    const sub = canonicalSubstation({
      connectionRequests: [
        ...
        withCoords(studiedRequest({
          id: 'accepted',
          offerStatus: 'offer_accepted',
          offerDates: { acceptedAt: '2026-03-01' },
          load: 4,
        }), 50.63),
        withCoords(connectedRequest({ id: 'connected', offerDates: { connectedAt: '2026-04-01' }, load: 5 }), 50.64),
        withCoords(connectedRequest({
          id: 'expired',
          offerDates: { connectedAt: '2025-01-01' },
          connectedRetentionMonths: 1,
          load: 6,
        }), 50.65),
        withCoords(canonicalRequest({ id: 'cancelled', customerStatus: 'cancelled', load: 7 }), 50.66),
      ],
    });
