...
describe('résumé de statut canonique', () => {
  ...
  it('qualifie une étude limitée comme conditionnelle', () => {
    const summary = buildRequestStatusSummary(
      studiedRequest({ loadStatus: 'LIMIT', loadPermanent: 3, loadFlexible: 2 }),
    );
  ...
  it('qualifie un raccordé maintenu puis libéré', () => {
    const kept = buildRequestStatusSummary(
      connectedRequest({ offerDates: { connectedAt: '2026-04-01' } }),
    );
    const ended = buildRequestStatusSummary(
      connectedRequest({
