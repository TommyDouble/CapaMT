...
export function calcCapacityN1(config) {
  ...
  const mtCap =
    config.mtBackup?.enabled && safeNum(config.mtBackup?.capacity, 0) > 0
      ? safeNum(config.mtBackup.capacity, 0)
      : 0;

  const n1 = remaining.reduce((s, p) => s + p, 0) * coeff + maxSecours * coeff + mtCap;
  return +n1.toFixed(1);
}
