...
import { getAssessment, getRequestedInjection, getRequestedLoad } from './requestModel.js';
...
export const reqClientPrelevTotal = (req) =>
  reqClientPrelevFerme(req) + reqClientPrelevFlexible(req);
...
export const reqNetTotal = (req) =>
  reqGrdPrelevFerme(req) +
  reqGrdPrelevFlexible(req) -
  (reqGrdInjFerme(req) + reqGrdInjFlexible(req));
...
export function buildConditionSummary(req, projects = [], isConditionDecision = false) {
  const ids = [
    ...
    ...(Array.isArray(req.assessment?.substation?.conditionedOnProjectIds)
      ? req.assessment.substation.conditionedOnProjectIds
      : []),
    ...(Array.isArray(req.assessment?.network?.conditionedOnProjectIds)
      ? req.assessment.network.conditionedOnProjectIds
      : []),
  ].filter((id, index, arr) => id && arr.indexOf(id) === index);
  ...
  const conditionLike =
    isConditionDecision || finalStatuses.includes('LIMIT') || finalStatuses.includes('FULL_FLEX');
