...
export const f1 = v => { const n = parseFloat(v); return isFinite(n) ? n.toFixed(1) : '—'; };
export const f2 = n => n != null ? n.toFixed(2) : '—';
export const pct  = n => n != null ? Math.round(n * 100) + '%' : '—';
export const pct1 = n => n != null ? (n * 100).toFixed(1) + '%' : '—';

export const uid = () => 'id-' + Date.now() + '-' + Math.random().toString(36).substr(2, 5);

export const statusLabel = s => ({
  'en_étude': 'En étude', 'engagé': 'Engagé', 'conditionnel': 'Conditionnel',
  'étudiée': 'Étudiée', 'raccordée': 'Raccordée', 'raccordé': 'Raccordé',
  'annulée': 'Annulée', 'annulé': 'Annulé',
  'planifié': 'Planifié', 'en_cours': 'En cours', 'validé': 'Validé',
}[s] || s);
