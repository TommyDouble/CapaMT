...
import { statusLabel, fmtShortDate } from '../../utils/format.js';

const BADGE_TOOLTIPS = {
  decision: {
    acceptable: 'Décision GRD: la capacité permanente demandée peut être accordée.',
    conditionnel:
      'Décision GRD: raccordement possible sous condition, généralement liée à un projet réseau ou à une limitation.',
    liste_attente:
      'Décision GRD: capacité indisponible à ce stade, dossier à maintenir en attente ou à retravailler.',
    en_analyse: 'Décision non encore finalisée.',
  },
  impact: {
    NONE: 'Aucun impact capacité actif.',
    QUEUE_RESERVED:
      'Dossier complet en file: la puissance demandée est réservée avant finalisation de l’étude.',
  ...
  split: {
    ...
    LIMIT:
      'Une partie de la puissance demandée peut être accordée en permanent; le solde est flexible.',
...
export function DecisionBadge({ decision, size = 'sm' }) {
  ...
  return (
    <span
      title={badgeTitle('decision', decision, 'en_analyse')}
...
export function ExpiryChip({ expiry }) {
  ...
  if (expiry.status === 'signé')
    return (
      <span
        title={`Convention signée le ${fmtShortDate(expiry.date)}.`}
        className="expiry-chip expiry-signed"
      >
        ✓ Conv. {fmtShortDate(expiry.date)}
      </span>
    );
  if (expiry.status === 'expiré')
    return (
      <span
        title={`Échéance expirée le ${fmtShortDate(expiry.date)}.`}
        className="expiry-chip expiry-expired"
      >
        ⚠ Expirée {fmtShortDate(expiry.date)}
      </span>
    );
  if (expiry.status === 'bientôt')
    return (
      <span
        title={`Échéance de réservation proche: ${expiry.daysLeft} jour(s) restant(s).`}
        className="expiry-chip expiry-soon"
      >
  ...
  return (
    <span
      title={`Échéance de réservation: ${fmtShortDate(expiry.date)}.`}
      className="expiry-chip expiry-ok"
    >
      🕐 {fmtShortDate(expiry.date)}
    </span>
  );
}

/** Pilule générique colorée. */
export function Pill({ children, style: extraStyle }) {
  return (
    <span
      style={{
...
export function Tag({ v }) {
  const c = TYPE_COLORS[v] ||
    STATUS_COLORS[v] || { bg: 'var(--slate)', color: 'var(--text-muted)', border: 'var(--border)' };
  return (
    <span
      title={`Type: ${statusLabel(v)}`}
      style={{
        display: 'inline-block',
        padding: '2px 8px',
        borderRadius: 20,
...
export function CapacityImpactChip({ impact, size = 'sm' }) {
  ...
  return (
    <span
      title={badgeTitle('impact', impact, 'NONE')}
      style={{ background: c.bg, color: c.color, border: `1px solid ${c.border}` }}
      className={`inline-flex items-center gap-1.5 rounded-full ${pad}`}
    >
      {c.label}
...
export const CustomerStatusBadge = (props) => (
  <ConfigBadge
    ...
    value={props.status}
    size={props.size}
    title={badgeTitle('customer', props.status, 'incomplete')}
  />
);

export const AssessmentStatusBadge = (props) => (
  <ConfigBadge
    config={ASSESSMENT_STATUS_CONFIG}
    fallback="not_started"
    value={props.status}
    size={props.size}
    title={badgeTitle('assessment', props.status, 'not_started')}
  />
);
...
export const OfferStatusBadge = (props) => (
  <ConfigBadge
    ...
    value={props.status}
    size={props.size}
    title={badgeTitle('offer', props.status, 'not_applicable')}
  />
);

export const CapacitySplitStatusBadge = (props) => (
  <ConfigBadge
    config={CAPACITY_SPLIT_CONFIG}
    fallback="PENDING"
    value={props.status}
    size={props.size}
    title={badgeTitle('split', props.status, 'PENDING')}
  />
);
...
export const ConfidenceBadge = (props) => (
  <ConfigBadge
    ...
    size={props.size}
    title={badgeTitle('confidence', props.confidence, 'LOW')}
  />
);
