    ...
    return (
      <>
        ...
        <span className="mono text-xs text-muted">
          Prél. {f1(getRequestedLoad(data))} MVA · Inj. {f1(getRequestedInjection(data))} MVA · MES {customer.requested?.year || '—'}
        </span>
      </>
    );
  }
        ...
        <div key={entry.id} className={`log-row ${i === 0 ? 'slide-down' : ''}`}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="flex items-center gap-2 flex-wrap">
              <span style={{ fontWeight: 600, color: 'var(--text-secondary)', fontSize: 11 }}>{entryTitle(entry)}</span>
            ...
            <div className="text-xs text-muted mt-0.5">
              {entry.subName && (
                <button
                  ...
                  style={{ color: 'var(--accent)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  {entry.subName}
                </button>
              )}
              {entry.subCode && <><span className="mx-1">·</span><span className="mono">{entry.subCode}</span></>}
              <span className="mx-1">·</span><span>{timeAgo(entry.timestamp)}</span>
            </div>
          </div>
          {entry.data?.id && (
            <button
              onClick={() => onDelete?.({ logId: entry.id, subId: entry.subId, dataId: entry.data.id })}
