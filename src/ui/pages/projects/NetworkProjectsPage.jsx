...
const INV_STATUS_PILL = {
  validé: 'pill-validé',
  en_cours: 'pill-en-cours',
  planifié: 'pill-planifié',
  annulé: 'pill-annulé',
};
...
export function NetworkProjectsPage({
  ...
  const atRisk = allSubstations.filter(
    (sub) => sub.status !== 'hors_service' && isSubstationAtRisk(sub, projects),
  );
  const atRiskInj = allSubstations.filter(
    (sub) => sub.status !== 'hors_service' && isSubstationAtRiskInjection(sub, projects),
  );
    ...
    <div className="space-y-5 fade-in">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Validés', val: validated, sub: 'budget engagé', color: 'var(--green)' },
          {
            label: 'À valider',
            val: planifié,
            sub: 'budget non encore engagé',
            color: planifié > 0 ? 'var(--amber)' : 'var(--green)',
          },
          {
            label: 'Enveloppe estimée',
            val: `${(totalCost / 1000).toFixed(1)} M€`,
            sub: `${totalCost.toLocaleString('fr')} k€ cumulés`,
            color: 'var(--text-primary)',
          },
        ].map((k) => (
          <div
            key={k.label}
            className="card"
            style={{ padding: 14, borderLeft: `3px solid ${k.color}` }}
          >
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {atRisk.map((sub) => {
              ...
              const satRisk = getFirstWithdrawalSaturationYear(
                sub,
                projects.filter((p) => p.status !== 'planifié'),
              );
              return (
                <div
                  key={sub.id}
                ...
                >
                  ...
                  <span style={{ fontSize: 10, color: 'var(--red)' }}>
                    sans : {satRisk || '>2035'} · avec : {satNominal || 'OK'}
                  </span>
                  <button
                    onClick={() => onNavigate(sub.id, 'evolution')}
      ...
      {atRiskInj.length > 0 && (
        ...
        >
          ...
          >
            Injection — {atRiskInj.length} SS dont la saturation inverse dépend d'un projet non
            validé
          </p>
          <div className="flex flex-wrap gap-3">
            {atRiskInj.map((sub) => {
              const satNom = getFirstInjectionSaturationYear(sub, projects);
              const satNo = getFirstInjectionSaturationYear(
                sub,
                projects.filter((p) => p.status !== 'planifié'),
              );
              return (
                ...
                >
                  <span style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{sub.name}</span>
                  <span style={{ fontSize: 10, color: 'var(--inj)' }}>
                    sans : {satNo || '>2035'} · avec : {satNom || 'OK'}
                  </span>
                  <button
                    onClick={() => onNavigate(sub.id, 'evolution')}
                    className="btn-edit-link"
      ...
      <div className="card" style={{ padding: 20 }}>
        <h3 className="font-semibold text-primary mb-1">
          Enveloppe budgétaire par année de mise en service
        </h3>
        <p className="text-xs text-muted mb-4">
          Empilé par statut · survol pour voir les projets de l'année
        </p>
        ...
        >
          ...
          {PROJ_TYPES.map((t) => (
            <option key={t} value={t}>
              {PROJ_TYPE_CONFIG[t].icon} {PROJ_TYPE_CONFIG[t].label}
            </option>
          ))}
        </select>
        <select
        ...
        <table className="w-full">
          <thead>
            <tr className="thead-row">
              <th className="text-left" style={{ minWidth: 240 }}>
                Projet
              </th>
          ...
          <tbody>
            ...
            {filtered.map((proj) => {
              ...
              return (
                <tr
                  key={proj.id}
                  className={`data-row ${proj.status === 'annulé' ? 'opacity-40' : ''}`}
                >
                  <td>
                    <div className="font-medium text-primary text-sm">{proj.name}</div>
                    {proj.notes && (
                      <div className="text-xs text-muted truncate max-w-xs">{proj.notes}</div>
                    )}
                  </td>
                  ...
                  <td className="text-right mono text-gray-700">
                    {proj.cost ? (
                      proj.cost.toLocaleString('fr')
                    ) : (
                  ...
                  <td>
                    <div className="flex flex-wrap gap-1">
                      {impactedSS.slice(0, 3).map((ssId) => {
                        ...
                        const isDecom = (proj.effects || []).some(
                          (e) => e.ssId === ssId && e.action === 'decommission',
                        );
                        return (
                          <button
                            key={ssId}
                            onClick={() => ss && onNavigate(ss.id, 'evolution')}
                            style={{
                      ...
                      {impactedSS.length > 3 && (
                        <span className="text-xs text-gray-400">+{impactedSS.length - 3}</span>
                      )}
                    </div>
                  </td>
                  <td className="text-right pr-3">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => setWizard(proj)} className="btn-edit-link text-xs">
                        Modifier
                      </button>
                      {proj.status === 'annulé' ? (
                        <button
                        ...
                        <button
                          onClick={() => onUpdateProject({ ...proj, status: 'annulé' })}
