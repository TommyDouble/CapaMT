    ...
    <div className="wizard-block wizard-block--suppression">
      ...
      <div className="wizard-block__body space-y-4">
        <FormRow label="Sous-station à décommissionner">
          <select
            value={block.ssId}
            onChange={(e) => onChange({ ...block, ssId: e.target.value })}
            className="input-field"
          >
            {substations
              .filter((s) => s.status !== 'hors_service')
              .map((s) => (
        ...
        {ss && (
          <div className="wizard-option-box">
            <p className="text-xs text-muted mb-1">
              Config actuelle de <strong>{ss.name}</strong> :
            </p>
        ...
        <FormRow
          label="Charge transférée vers une autre SS (± MVA)"
          hint="Saisir la valeur négative ici (charge sortant de cette SS)"
        >
          <input
            type="number"
            step="0.5"
        ...
        <div className="alert-box alert-box--danger">
          <p className="text-xs text-red-700 font-semibold">
            ⚠ Cette SS sera marquée <strong>hors service</strong> à la MES du projet. Elle reste
            visible en lecture seule avec un marqueur visuel. Ses demandes de raccordement devront
            être réaffectées manuellement.
          </p>
        </div>
      </div>
    </div>
