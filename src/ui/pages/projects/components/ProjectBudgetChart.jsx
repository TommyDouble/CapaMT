  ...
  useEffect(() => {
    ...
    projects.forEach((p) => {
      if (YEARS.includes(p.year)) {
        const st = ['validé', 'en_cours', 'planifié', 'annulé'].includes(p.status)
          ? p.status
          : 'planifié';
    ...
    chartRef.current = new Chart(canvasRef.current.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Validé',
            data: YEARS.map((y) => byYear[y].validé),
            backgroundColor: STATUS_COLORS.validé,
            stack: 'b',
          },
          {
            label: 'En cours',
            data: YEARS.map((y) => byYear[y].en_cours),
            backgroundColor: STATUS_COLORS.en_cours,
            stack: 'b',
          },
          {
            label: 'Planifié',
            data: YEARS.map((y) => byYear[y].planifié),
            backgroundColor: STATUS_COLORS.planifié,
            stack: 'b',
          },
          {
            label: 'Annulé',
            data: YEARS.map((y) => byYear[y].annulé),
            backgroundColor: STATUS_COLORS.annulé,
            stack: 'b',
          },
        ],
      },
      options: {
        ...
        scales: {
          x: {
            stacked: true,
            grid: { color: t.grid },
            ticks: { font: { family: 'JetBrains Mono', size: 11 }, color: t.text },
          },
          y: {
            stacked: true,
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              afterBody: (items) => {
                ...
                const total = Object.values(byYear[y])
