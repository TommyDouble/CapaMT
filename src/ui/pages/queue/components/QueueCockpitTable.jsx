...
const CAPAC_ACTION_STATUS = {
  ...
  partial: {
    ...
    color: '#92400e',
    detail: 'Un retour CAPAC a été encodé pour un sens, mais tous les sens applicables ne sont pas encore répondus.',
  },
};
...
function TableHeader({ filters, sort, onOpenMenu }) {
  const filterActive = key => key === 'client'
    ? !!filters.client?.trim()
    : (filters[key] || []).length > 0;
  const sortMark = field => sort.field === field ? (sort.direction === 'asc' ? '↑' : '↓') : null;
    ...
    <thead>
      <tr>
        <th style={cellStyle('center', 72)}>
          <HeaderButton active={sort.field === 'priority'} sort={sortMark('priority')} align="center" onClick={e => onOpenMenu('priority', e, 'sort')}>
            Priorité
          </HeaderButton>
        </th>
        <th style={cellStyle('center', 104)}>
          <HeaderButton active={filterActive('fifo') || sort.field === 'fifoRank'} sort={sortMark('fifoRank')} align="center" onClick={e => onOpenMenu('fifo', e, 'filterSort')}>
            Rang FIFO
          </HeaderButton>
        </th>
        <th style={cellStyle('left', 170)}>
          <HeaderButton active={filterActive('substations')} onClick={e => onOpenMenu('substations', e, 'filter')}>
            Sous-station
          </HeaderButton>
        </th>
        <th style={cellStyle('left', 230)}>
          <HeaderButton active={filterActive('client') || sort.field === 'customer'} sort={sortMark('customer')} onClick={e => onOpenMenu('client', e, 'client')}>
            Demandeur
          </HeaderButton>
        </th>
        ...
        <th style={cellStyle('center', 104)}>
          <HeaderButton active={filterActive('directions')} onClick={e => onOpenMenu('directions', e, 'filter')} align="center">
            Sens
          </HeaderButton>
        </th>
        <th style={cellStyle('right', 164)}>
          <HeaderButton active={sort.field === 'capacity'} sort={sortMark('capacity')} align="right" onClick={e => onOpenMenu('capacity', e, 'sort')}>
            Puissance
          </HeaderButton>
        </th>
        <th style={cellStyle('center', 164)}>Contrainte / condition</th>
        <th style={cellStyle('center', 128)}>
          <HeaderButton active={filterActive('reservations')} onClick={e => onOpenMenu('reservations', e, 'filter')} align="center">
            Réservation
          </HeaderButton>
        </th>
        <th style={cellStyle('center', 128)}>
          <HeaderButton active={filterActive('decisions')} onClick={e => onOpenMenu('decisions', e, 'filter')} align="center">
            Décision
          </HeaderButton>
        </th>
        <th style={cellStyle('center', 120)}>
          <HeaderButton active={sort.field === 'deadline'} sort={sortMark('deadline')} align="center" onClick={e => onOpenMenu('deadline', e, 'sort')}>
            Échéance
          </HeaderButton>
        </th>
...
function ConstraintBadge({ value }) {
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 700, color: value === 'UNKNOWN' ? 'var(--text-muted)' : 'var(--text-secondary)' }}>
      {label}
    </span>
  );
}
...
function FifoRankCell({ row }) {
  ...
  const color = row.isHeadOfQueue ? 'var(--accent)' : inQueue ? 'var(--text-secondary)' : 'var(--text-muted)';
  return (
    <td style={{ textAlign: 'center', padding: '11px 8px' }}>
      ...
      {row.isHeadOfQueue && (
        <div style={{ fontSize: 8, color: 'var(--accent)', fontWeight: 800, marginTop: 2, textTransform: 'uppercase', letterSpacing: '.05em' }}>
          tête
        </div>
      )}
    </td>
...
function ConstraintConditionCell({ row }) {
  ...
  const hasConstraint = row.technicalResult && row.limitingConstraint && row.limitingConstraint !== 'UNKNOWN';
  return (
    <td style={{ textAlign: 'center', padding: '11px 8px' }}>
      {row.technicalResult ? (
        <ConstraintBadge value={row.limitingConstraint} />
      ) : (
        <EmptyValue />
      )}
      {summary && (
...
function PriorityCell({ row }) {
  const color = row.urgency >= 95 ? 'var(--red)'
    : row.urgency >= 85 ? '#ea580c'
      : row.urgency >= 70 ? 'var(--amber)'
        : row.urgency >= 55 ? 'var(--accent)'
          : 'var(--text-muted)';
  const label = row.urgency >= 95 ? 'P0'
    : row.urgency >= 85 ? 'P1'
      : row.urgency >= 70 ? 'P2'
        : row.urgency >= 55 ? 'P3'
          : 'P4';
...
function SubstationCell({ row, onNavigate }) {
  return (
    <td style={{ padding: '11px 10px' }}>
      <button type="button" onClick={() => onNavigate(row.sub.id, 'demandes')} style={{
        color: 'var(--accent)',
        fontSize: 12,
        fontWeight: 800,
        background: 'none',
      ...
      <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{row.substationCode}</div>
    </td>
  );
}
...
function PowerCell({ row }) {
  ...
  if (!row.technicalResult) {
    if (!hasRequestedLoad && !hasRequestedInjection) {
      return <td style={{ textAlign: 'right', padding: '11px 10px', color: 'var(--text-muted)', fontSize: 11 }}>—</td>;
    }
    return (
      <td style={{ textAlign: 'right', padding: '11px 10px' }}>
        {hasRequestedLoad && (
          <div className="mono" style={{ fontSize: 11, color: 'var(--prelev)', fontWeight: 800 }}>
            P {f1(row.requestedLoad)} <span style={{ color: 'var(--text-muted)', fontWeight: 600 }}>MVA demandé</span>
          </div>
        )}
        {hasRequestedInjection && (
          <div className="mono" style={{ fontSize: 11, color: 'var(--inj)', fontWeight: 800 }}>
            I {f1(row.requestedInjection)} <span style={{ color: 'var(--text-muted)', fontWeight: 600 }}>MVA demandé</span>
          </div>
        )}
      </td>
    );
  ...
  return (
    <td style={{ textAlign: 'right', padding: '11px 10px' }}>
      {hasTechnicalLoad && (
        <div className="mono" style={{ fontSize: 11, color: 'var(--prelev)', fontWeight: 800 }}>
          P {f1(row.permanentLoad)} / {f1(row.flexibleLoad)} <span style={{ color: 'var(--text-muted)', fontWeight: 600 }}>MVA</span>
        </div>
      )}
      {hasTechnicalInjection && (
        <div className="mono" style={{ fontSize: 11, color: 'var(--inj)', fontWeight: 800 }}>
          I {f1(row.permanentInjection)} / {f1(row.flexibleInjection)} <span style={{ color: 'var(--text-muted)', fontWeight: 600 }}>MVA</span>
        </div>
      )}
    </td>
  );
    ...
    <td style={{ textAlign: 'center', padding: '11px 8px' }}>
      {row.displayReservationStatus ? (
        <>
          ...
          {row.expiry?.status === 'bientôt' && <div style={{ marginTop: 3 }}><ExpiryChip expiry={row.expiry} /></div>}
        </>
      ) : <EmptyValue />}
    </td>
  );
...
function DecisionCell({ row }) {
  return (
    <td style={{ textAlign: 'center', padding: '11px 8px' }}>
      {row.displayDecision ? <DecisionBadge decision={row.displayDecision} size="xs" /> : <EmptyValue />}
    </td>
  );
}
...
function actionLabel(row) {
  ...
  if (row.offerStatus === 'offer_expired' || row.displayReservationStatus === 'expired') return 'Traiter expiration';
  if (row.stepKey === 'to_connect') return 'Raccorder';
  if (row.stepKey === 'offer_action' && row.offerStatus === 'not_applicable') return 'Formuler offre';
...
function ActionCell({ row }) {
  ...
  return (
    <span
      ...
      onFocus={showTooltip}
      onBlur={hideTooltip}
      style={{ display: 'inline-flex', alignItems: 'flex-start', flexDirection: 'column', gap: 3, fontSize: 12, fontWeight: 800, color: actionColor(row) }}
    >
...
function ActionTooltip({ position, labels, capacDetail }) {
  return (
    ...
    >
      ...
      {capacDetail && (
        <span className="queue-action-tooltip__note">{capacDetail}</span>
      )}
    </span>
  );
}
...
function DossierButton({ row, onNavigateToRequest }) {
  ...
  return (
    <td style={{ textAlign: 'right', padding: '11px 12px 11px 8px' }}>
      <button type="button" disabled={disabled} onClick={() => onNavigateToRequest?.(row.sub.id, row.req.id)} style={{
...
function EmptyRow({ label }) {
  return (
    <tr>
      <td colSpan={13} style={{ textAlign: 'center', padding: 42, color: 'var(--text-muted)', fontSize: 13 }}>
        {label}
      </td>
    </tr>
  );
  ...
  return (
    <div className="card" style={{ overflow: 'hidden' }}>
      <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ width: 9, height: 9, borderRadius: 999, background: activeStepConfig.color }} />
        <p style={{ fontSize: 13, fontWeight: 900, color: 'var(--text-primary)' }}>{activeStepConfig.label}</p>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1380 }}>
          ...
          />
          <tbody>
            {displayedRows.length === 0 && <EmptyRow label="Aucune demande ne correspond à cette vue." />}
            {displayedRows.map((row, i) => {
              const step = STEP_BY_KEY[row.stepKey] || STEP_BY_KEY.to_complete;
              return (
                <tr key={row.id} className="data-row stagger-item" style={{
                  animationDelay: `${i * 10}ms`,
                  boxShadow: `inset 3px 0 0 ${step.color}`,
                  opacity: row.isClosed ? .62 : 1,
                }}>
                  ...
                  <td style={{ padding: '11px 10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 700, fontSize: 12, color: row.isClosed ? 'var(--text-muted)' : 'var(--text-primary)' }}>
                        {row.customerName}
                      </span>
                      <Tag v={row.type} />
                    </div>
                    {row.reference && <div style={{ fontSize: 9, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', marginTop: 2 }}>{row.reference}</div>}
                  </td>
                  <td style={{ textAlign: 'center', padding: '11px 8px' }}><StepBadge row={row} /></td>
                  <td style={{ padding: '11px 10px' }}>
                    <ActionCell row={row} />
                  </td>
                  <td style={{ textAlign: 'center', padding: '11px 8px' }}><DirectionChip direction={row.direction} /></td>
                  <PowerCell row={row} />
