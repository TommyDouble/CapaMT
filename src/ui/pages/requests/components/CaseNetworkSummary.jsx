...
function ResidualBar({ label, value, capacity, color }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'var(--bg-muted)', overflow: 'hidden' }}>
        <div style={{ width: `${pct * 100}%`, height: '100%', background: c, borderRadius: 4, transition: 'width .3s' }} />
      </div>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: c, minWidth: 60, textAlign: 'right' }}>
...
export function CaseNetworkSummary({ req, sub, projects, queueItem }) {
  ...
  const sparkVals = YEARS.map(y => {
    ...
    const committed = queue
      ...
      .reduce((s, item) => s + getEffectiveRigidReservation(item.req) * getFoisonnement(item.req, sub), 0);
  ...
  return (
    <div className="card" style={{ padding: '14px 18px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {/* Résiduels */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 10 }}>
            Résiduel à l'année de la demande ({reqYear})
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {wBefore != null ? (
              <>
                <ResidualBar label="Prél. avant demande" value={wBefore} capacity={capDirN1} color="var(--prelev)" />
                <ResidualBar label="Prél. après demande" value={wAfter} capacity={capDirN1} color="var(--prelev)" />
              </>
            ) : (
            ...
            {iBefore != null && capRevN1 > 0 && (
              <>
                <ResidualBar label="Inj. avant demande" value={iBefore} capacity={capRevN1} color="var(--inj)" />
                {iAfter != null && <ResidualBar label="Inj. après demande" value={iAfter} capacity={capRevN1} color="var(--inj)" />}
              </>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
            <div style={{ padding: '4px 10px', background: 'var(--slate)', borderRadius: 6, fontSize: 11 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
            ...
            <div style={{ fontSize: 9, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              {YEARS.map((y, i) => (
                <div key={y} style={{ display: 'flex', justifyContent: 'space-between', gap: 8,
                  color: sparkVals[i] < 0 ? 'var(--red)' : sparkVals[i] < 3 ? 'var(--amber)' : 'var(--text-muted)',
                  fontWeight: y === reqYear ? 800 : 400 }}>
                  <span>{y}{y === reqYear ? ' ←' : ''}</span>
      ...
      <div style={{ marginTop: 18, borderTop: '1px solid var(--border)', paddingTop: 14 }}>
        <p style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.07em',
          color: condMissing ? 'var(--amber)' : 'var(--text-muted)', marginBottom: 10 }}>
          Projets réseau conditionnants
        </p>

        {condMissing && (
          <div style={{ background: 'var(--amber-dim)', border: '1px solid rgba(217,119,6,.25)',
        ...
        {condProjects.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {condProjects.map(proj => {
              const statusColors = {
                planifié: { color: 'var(--accent-muted)', bg: 'var(--accent-soft)', border: 'var(--accent-border)' },
              ...
              return (
                <div key={proj.id} style={{ display: 'flex', alignItems: 'center', gap: 12,
                  padding: '8px 12px', background: 'var(--slate)', borderRadius: 8,
                  border: '1px solid var(--border)' }}>
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>{proj.name}</span>
                    {proj.notes && <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{proj.notes}</p>}
                  </div>
