...
import { makeCapacitySplit, pendingSplit } from '../../../../engines/capacitySplit.js';
...
import {
  computeSubstationSplit,
  evaluateRequestCapacity,
} from '../../../../engines/capacityEvaluation.js';
import {
  canEditAssessment,
  canEditOffer,
...
export function CapacitySplitCard({ title, split, compact = false }) {
  ...
  return (
    <div
      className="card"
      style={{ padding: compact ? 12 : 14, borderTop: `3px solid ${statusStyle.color}` }}
    >
      <div
      ...
      {missing ? (
        ...
        split.reason && (
          <p
            style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 10, lineHeight: 1.4 }}
          >
            {split.reason}
          </p>
        )
...
function Metric({ label, value, color = 'var(--text-primary)' }) {
  return (
    <div>
      ...
      <div className="mono" style={{ fontSize: 17, fontWeight: 800, color }}>
        {f1(value)} <span style={{ fontSize: 10, fontWeight: 500 }}>MVA</span>
      </div>
    </div>
  );
}
...
export function DecisionBanner({ req }) {
  ...
  return (
    <div
      className="card"
      style={{ padding: 18, borderLeft: `4px solid ${c.color}`, background: c.bg }}
    >
      <div
        style={{
          display: 'flex',
...
function splitToForm(split) {
  ...
  return {
    ...
    reason:
      split?.status === 'PENDING' && defaultPendingReasons.has(split?.reason)
        ? ''
        : split?.reason || '',
...
function formToSplit(form, requested, source, defaultReason, fallbackAnsweredAt = '') {
  ...
  if (form.status === 'PENDING')
    return pendingSplit(requested, source, form.reason || defaultReason);
  if (form.status === 'KO')
  ...
  const permanent =
    form.status === 'OK' ? requested : form.status === 'FULL_FLEX' ? 0 : safeNum(form.permanent, 0);
  return makeCapacitySplit({
    requested,
    permanent,
    source,
...
export function TechnicalAssessmentPanel({ req, sub, projects, onSave }) {
  ...
  const [subProjectIds, setSubProjectIds] = useState(
    assessment.substation?.conditionedOnProjectIds || [],
  );
  const [netProjectIds, setNetProjectIds] = useState(
    assessment.network?.conditionedOnProjectIds || req.conditionedOnProjectIds || [],
  );
  ...
  const updateForm = (setter, key, value) =>
    setter((prev) => ({ ...prev, [key]: key === 'answeredAt' ? clampActualDate(value) : value }));

  const updateUpstreamForm = (setter, key, value) =>
    setter((prev) => {
  ...
  const buildUpstreamLayer = (
    fallbackAnsweredAt = capac.receivedAt,
    loadForm = upLoad,
    injectionForm = upInj,
  ) => ({
    load: formToSplit(
  ...
  const buildEdited = ({
    ...
    let upstream = buildUpstreamLayer(
      capacForm.receivedAt,
      upstreamLoadForm,
      upstreamInjectionForm,
    );
    let capacTracking = updateCapacTrackingForUpstream(
      capacForm,
      upstream,
      requestedLoad,
      requestedInjection,
    ...
    if (capacTracking.receivedAt) {
      upstream = buildUpstreamLayer(
        capacTracking.receivedAt,
        upstreamLoadForm,
        upstreamInjectionForm,
      );
      capacTracking = updateCapacTrackingForUpstream(
        capacTracking,
        upstream,
        requestedLoad,
        requestedInjection,
    ...
    const conditionedOnProjectIds = [...subProjectIds, ...netProjectIds].filter(
      (projectId, index, arr) => projectId && arr.indexOf(projectId) === index,
    );
  ...
  const upstreamComplete = isUpstreamResponseComplete(
    buildUpstreamLayer(),
    requestedLoad,
    requestedInjection,
  );
  ...
  return (
    <div className="card" style={{ padding: 18 }}>
      ...
      >
        <CapacitySplitCard
          title="Final prélèvement"
          split={displayFinalSplit(preview.assessment, preview.assessment.final?.load)}
          compact
        />
        <CapacitySplitCard
          title="Final injection"
          split={displayFinalSplit(preview.assessment, preview.assessment.final?.injection)}
          compact
        />
      ...
      >
        ...
        >
          ...
          <select
            ...
            onChange={(e) => setScenarioProfile(e.target.value)}
          >
            {Object.entries(SCENARIO_PROFILE_CONFIG).map(([key, cfg]) => (
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn-secondary" disabled={!editable} onClick={handleSaveStudy}>
            Enregistrer étude
          </button>
          <button
            className="btn-primary"
            disabled={!editable || !canFinalizeAssessment(preview)}
...
function ConditionProjectSelector({
  projects = [],
  substationId,
  selectedIds = [],
  disabled,
  ...
  const otherProjects = activeProjects.filter(
    (project) => !relevantProjects.some((relevant) => relevant.id === project.id),
  );
      ...
      >
        ...
        <span>
          ...
          <span style={{ display: 'block', fontSize: 9, color: 'var(--text-muted)', marginTop: 2 }}>
            ...
            {outsideSubstation ? ' · hors sous-station' : ''}
          </span>
        </span>
      </label>
    );
  ...
  return (
    <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border)' }}>
      ...
      {activeProjects.length === 0 ? (
        <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>Aucun projet réseau disponible.</p>
      ) : (
        <div style={{ display: 'grid', gap: 6 }}>
          {relevantProjects.length > 0 ? (
            relevantProjects.map(renderProject)
          ) : (
...
function CapacTrackingEditor({
  ...
  const updateDate = (field, value) =>
    onChange((prev) => {
      ...
      if (field === 'receivedAt')
        next.status = value ? 'RECEIVED' : next.sentAt ? 'SENT' : 'NOT_SENT';
      if (field === 'sentAt' && next.status !== 'RECEIVED')
        next.status = value ? 'SENT' : 'NOT_SENT';
      return next;
    });
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <input
          ...
          disabled={disabled}
          value={capac.sentAt || ''}
          onChange={(e) => updateDate('sentAt', e.target.value)}
          title="Date d'envoi de la demande CAPAC"
        />
        <input
          className="input-field"
          type="date"
          max={TODAY()}
          disabled={disabled}
          value={capac.receivedAt || ''}
          onChange={(e) => updateDate('receivedAt', e.target.value)}
          title="Date de réception complète de la réponse CAPAC"
        />
      </div>
      <div
        style={{
      ...
      >
        <p style={{ fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.35 }}>
          {upstreamComplete
            ? 'Réponse amont complète: passage en CAPAC reçue à l’enregistrement.'
            : 'Dates auditables: demande envoyée puis réponse ELIA reçue.'}
        </p>
...
function CapacReturnModal({
  ...
  const updateLoad = (key, value) =>
    setLoadDraft((prev) => ({
      ...prev,
      [key]: key === 'answeredAt' ? clampActualDate(value) : value,
    }));
  const updateInjection = (key, value) =>
    setInjectionDraft((prev) => ({
      ...prev,
      [key]: key === 'answeredAt' ? clampActualDate(value) : value,
    }));
  ...
  const handleSave = () => {
    ...
    const nextCapac = updateCapacTrackingForUpstream(
      capac,
      upstream,
      requestedLoad,
      requestedInjection,
        ...
        {requestedInjection > 0 && (
          <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
            ...
            <SplitInput
              label="Réponse"
              form={injectionDraft}
              onChange={updateInjection}
              disabled={false}
            />
          </div>
        )}
        <p style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.45 }}>
          Une réponse partielle complète uniquement le sens encodé. Le suivi CAPAC passe en reçue
          lorsque tous les sens applicables sont renseignés.
        </p>
      </div>
    </ModalShell>
  );
...
function AssessmentResultCard({ req }) {
  ...
  const status = pending
    ? 'PENDING'
    : statuses.includes('KO')
      ? 'KO'
      : statuses.includes('LIMIT') || statuses.includes('FULL_FLEX')
        ? 'LIMIT'
        : 'OK';
  const c = CAPACITY_SPLIT_CONFIG[status] || CAPACITY_SPLIT_CONFIG.PENDING;
  ...
  const action = actions.length
    ? actions.join(' · ')
    : pending
      ? 'Compléter les couches manquantes'
      : 'Prêt à finaliser';
    ...
    <div className="card" style={{ padding: 12, borderTop: `3px solid ${c.color}` }}>
      ...
      >
        <p>
          ...
          {isQualifiedLimitingConstraint(assessment)
            ? constraintLabel(assessment.final?.limitingConstraint)
            : 'À déterminer'}
        </p>
        <p>
        ...
        {assessment.confidence && (
          <p>
            ...
            <span
              title={confidenceTooltip(assessment)}
    ...
    >
      <div style={{ display: 'grid', gap: 14 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8 }}>
          ...
          <WizardMetric
            label="Puissance"
            value={`P ${f1(requestedLoad)} / I ${f1(requestedInjection)} MVA`}
          />
        </div>
        ...
        <DiagnosticsList
          title="Projets sécurisés comptés"
          rows={d.countedProjects || []}
          empty="Aucun projet sécurisé compté."
          type="project"
        />
        <DiagnosticsList
          title="Projets exclus"
          rows={d.excludedProjects || []}
          empty="Aucun projet poste exclu."
          type="project"
          muted
        />
        <DiagnosticsList
          title="Engagements actifs comptés"
          rows={activeCommitments}
          empty="Aucun engagement actif déjà acquis/réservé."
        />
        <DiagnosticsList
          title="Demandes FIFO antérieures comptées"
          rows={fifoPrevious}
          empty="Aucune demande antérieure ne consomme la marge avant ce dossier."
        />
        <DiagnosticsList
          title="Demandes exclues"
          rows={excluded}
          empty="Aucune demande exclue."
          muted
        />
      </div>
    </ModalShell>
...
function WizardMetric({ label, value }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, color: 'var(--text-primary)', fontWeight: 800, marginTop: 4 }}>
        {value}
      </div>
    </div>
  );
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
      ...
      >
        ...
        {split ? (
          ...
          <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Non applicable</span>
        )}
      </div>
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
      <div
        style={{
          padding: '8px 10px',
          background: muted ? 'var(--bg-muted)' : 'var(--bg-raised)',
      ...
      {!rows?.length ? (
        ...
        <div style={{ display: 'grid' }}>
          {rows.map((row) => (
            <div
              key={`${type}-${row.id}-${row.reason}`}
              style={{
                display: 'grid',
                gridTemplateColumns:
                  type === 'project' ? '1.6fr .7fr .7fr 1fr' : '1.6fr .8fr .8fr .8fr .8fr 1fr',
                gap: 8,
                padding: '7px 10px',
      ...
      >
        {['PENDING', 'OK', 'LIMIT', 'FULL_FLEX', 'KO'].map((status) => (
          <option key={status} value={status}>
            {CAPACITY_SPLIT_CONFIG[status].label}
          </option>
        ))}
      </select>
      <input
        className="input-field"
        disabled={disabled || ['OK', 'PENDING', 'KO', 'FULL_FLEX'].includes(form.status)}
        type="number"
        min="0"
        step=".1"
        value={form.permanent}
        placeholder="Permanent MVA"
        onChange={(e) => onChange('permanent', e.target.value)}
      />
      <span />
      <input
        className="input-field"
        disabled={disabled}
        value={form.reason}
        placeholder="Commentaire / référence"
...
export function OfferStatusModal({ req, onSave, onClose }) {
  ...
  const [date, setDate] = useState(
    offer.status === 'offer_connected' ? offer.connectedAt || TODAY() : TODAY(),
  );
  ...
  return (
    <ModalShell
      ...
      subtitle="Cycle commercial séparé de l’étude technique"
      onClose={onClose}
      footer={
        <>
          <button className="btn-secondary" onClick={onClose}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <FormRow label="Date de changement">
          <input
            ...
            value={date}
            disabled={!canEditOffer(req)}
            onChange={(e) => setDate(clampActualDate(e.target.value))}
          />
        </FormRow>
      </div>
