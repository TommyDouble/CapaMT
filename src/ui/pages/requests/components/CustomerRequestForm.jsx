...
export function CustomerRequestForm({ req, sub, onSave, onClose }) {
  ...
  const setCoordinates = (key, value) =>
    setForm((prev) => ({ ...prev, coordinates: { ...prev.coordinates, [key]: value } }));
  const setPowerBreakdown = (patch) =>
    setForm((prev) => ({
      ...prev,
      powerBreakdown:
        typeof patch === 'function'
          ? patch(prev.powerBreakdown)
          : { ...prev.powerBreakdown, ...patch },
    }));

  const summary = computePowerBreakdownSummary(form.powerBreakdown, form);
  const effectiveLoad =
    form.powerBreakdown.loadMode === 'AUTO' ? summary.loadSum : safeNum(form.load, 0);
  const effectiveInjection =
    form.powerBreakdown.injectionMode === 'AUTO'
      ? summary.injectionSum
      : safeNum(form.injection, 0);
  ...
  const handleSave = () => {
    ...
    const injection =
      powerBreakdown.injectionMode === 'AUTO'
        ? saveSummary.injectionSum
        : safeNum(form.injection, 0);
    const nextErrors = {};
    ...
    const completeAfterSave = Boolean(
      ...
      sub.id &&
      load + injection > 0 &&
      (form.desiredCommissioningDate || form.year),
    );
    const readyForStudyAt =
      form.readyForStudyAt || (completeAfterSave ? form.requestDate : customer.readyForStudyAt);
        ...
        customer: {
          ...
          site: {
            ...
            coordinates: {
              ...
              source:
                form.coordinates.lat || form.coordinates.lng ? 'manual' : form.coordinates.source,
            },
          },
          requested: {
            ...requested,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Section title="Dates et priorité" color="var(--accent)" defaultOpen>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <FormRow
            ...
            hint="Date audit de réception de la demande client."
          >
            <input
              className="input-field"
              type="date"
          ...
          <FormRow
            label="Date de complétude / recevabilité"
            hint="Détermine la position FIFO; auto-remplie si vide."
          >
            <input
              className="input-field"
              type="date"
              value={form.readyForStudyAt?.slice?.(0, 10) || ''}
              onChange={(e) => set('readyForStudyAt', e.target.value)}
            />
          </FormRow>
          ...
          <FormRow label="Date MES souhaitée">
            <input
              className="input-field"
              type="date"
              value={form.desiredCommissioningDate || ''}
              onChange={(e) => set('desiredCommissioningDate', e.target.value)}
            />
          </FormRow>
        </div>
      </Section>
      ...
      <Section title="Site / adresse" color="var(--green)" defaultOpen={false}>
        ...
        >
          ...
          <button
            ...
            type="button"
            onClick={() => setAddressWizardOpen(true)}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr .6fr .7fr 1fr', gap: 10 }}>
          ...
          <FormRow label="Adresse libre">
            <input
              className="input-field"
              value={form.address.freeform}
              onChange={(e) => setAddress('freeform', e.target.value)}
            />
          </FormRow>
        </div>
      </Section>
      ...
      <Section
        title="Puissances demandées"
        badge={errors.power ? 'Puissance requise' : null}
        color="var(--prelev)"
        defaultOpen
      >
        {errors.power && (
        ...
        >
          ...
          >
            <span>
              Prélèvement demandé : <strong className="mono">{f1(effectiveLoad)} MVA</strong>
            </span>
            <span>
              Injection demandée : <strong className="mono">{f1(effectiveInjection)} MVA</strong>
            </span>
            <span>
              Total contractuel :{' '}
              <strong className="mono">{f1(effectiveLoad + effectiveInjection)} MVA</strong>
            </span>
          </div>
        </div>
      </Section>
...
function PowerBreakdownEditor({
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
      ...
      >
        <p
          style={{ fontSize: 12, fontWeight: 800, color: isLoad ? 'var(--prelev)' : 'var(--inj)' }}
        >
          {title}
        </p>
        <div
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {items.map((item) => (
          <div
            ...
            style={{
              display: 'grid',
              gridTemplateColumns: '1fr 1.1fr .8fr .75fr 26px',
              gap: 6,
              alignItems: 'center',
            }}
          >
            <select
              className="input-field"
              value={item[mainKey] || options[0]}
              onChange={(e) => setItem(item.id, mainKey, e.target.value)}
            >
              {options.map((option) => (
            ...
            <input
              ...
              value={item.powerMva ?? ''}
              placeholder="MVA"
              onChange={(e) => setItem(item.id, 'powerMva', e.target.value)}
            />
            <label
              style={{
                display: 'flex',
            ...
            >
              <input
                type="checkbox"
                checked={Boolean(isLoad ? item.flexible : item.curtailable)}
                onChange={(e) =>
                  setItem(item.id, isLoad ? 'flexible' : 'curtailable', e.target.checked)
                }
              />
      ...
      <button
        ...
        onClick={add}
        className="btn-secondary"
        style={{ width: '100%', marginTop: 8, fontSize: 11, padding: '5px 10px' }}
      >
      ...
      >
        ...
        <FormRow
          label="Total demandé"
          hint={mode === 'AUTO' ? 'Calculé automatiquement' : 'Override contractuel autorisé'}
        >
          <input
            className="input-field"
            type="number"
      ...
      {deltaVisible && (
        ...
        >
          Écart assumé: total demandé {f1(total)} MVA vs composants {f1(sum)} MVA (
          {delta > 0 ? '+' : ''}
          {f1(delta)} MVA).
        </p>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr .6fr .7fr 1fr 1fr', gap: 10 }}>
          <FormRow label="Rue">
            <input
              className="input-field"
              value={parsed.street}
              onChange={(e) => updateParsed('street', e.target.value)}
            />
          </FormRow>
          <FormRow label="N°">
            <input
              className="input-field"
              value={parsed.number}
              onChange={(e) => updateParsed('number', e.target.value)}
            />
          </FormRow>
          <FormRow label="CP">
            <input
              ...
              onChange={(e) => updateParsed('postalCode', e.target.value)}
            />
          </FormRow>
          <FormRow label="Commune">
            <input
              className="input-field"
              value={parsed.city}
              onChange={(e) => updateParsed('city', e.target.value)}
            />
          </FormRow>
          <FormRow label="Pays">
            <input
              ...
              onChange={(e) => updateParsed('country', e.target.value)}
            />
          </FormRow>
        </div>
      </div>
