...
function addStep(steps, step) {
  ...
  steps.push({
    ...
    sortTime:
      hasTime(step.date) && !invalidFuture ? timeStamp(step.date) : Number.POSITIVE_INFINITY,
...
function desiredCommissioning(req, customer) {
  return customer.requested?.desiredCommissioningDate || customer.requested?.year || '';
}
...
function buildMilestones(req) {
  ...
  if (
    ...
    expiry?.date &&
    ['bientôt', 'expiré'].includes(expiry.status)
  ) {
    addStep(steps, {
...
export function CaseTimeline({ req, sub, onUpdate }) {
  ...
  const handleAddMilestone = () => {
    ...
    const updated = {
      ...
      milestones: [
        ...(req.milestones || []),
        { id: uid(), ...newMilestone, label: newMilestone.label.trim() },
      ],
    };
  ...
  return (
    <div className="card" style={{ padding: '14px 18px' }}>
      ...
      <div style={{ overflowX: 'auto', paddingBottom: 8 }}>
        <div
          style={{
            ...
            gap: 0,
            minWidth: Math.max(1, milestones.length) * 118,
          }}
        >
            ...
            <React.Fragment key={step.id}>
              <div style={{ position: 'relative' }}>
                ...
                {!step.fixed && (
                  <button
                    type="button"
                    onClick={() => handleRemoveMilestone(step.id)}
                    style={{
                      position: 'absolute',
                      top: -2,
                      right: -4,
      ...
      {showAddMilestone && (
        ...
        >
          <p style={{ fontSize: 11, fontWeight: 800, color: 'var(--accent)', marginBottom: 8 }}>
            Nouveau jalon manuel
          </p>
