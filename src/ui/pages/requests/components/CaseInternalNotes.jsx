...
export function CaseInternalNotes({ req, sub, onUpdate }) {
  ...
  const handleSave = () => {
    if (draft === req.internalNotes) { setEditing(false); return; }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        {!editing && (
          <button type="button" onClick={() => setEditing(true)}
            className="btn-edit-link" style={{ fontSize: 11 }}>
      ...
      {editing ? (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <button type="button" onClick={handleSave} className="btn-primary" style={{ fontSize: 11, padding: '4px 14px' }}>
              ✓ Enregistrer
            </button>
            <button type="button" onClick={handleCancel} className="btn-secondary" style={{ fontSize: 11, padding: '4px 14px' }}>
              Annuler
            </button>
          </div>
        </div>
