...
import {
  StatusPhaseBadge, Tag,
} from '../../../shared/badges.jsx';

export function CaseHeader({ req, sub, queueItem, onEdit, onBack, prevViewLabel, editDisabled = false, editLabel = 'Modifier' }) {
        ...
        <div className="flex items-start justify-between gap-6">
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
              <h2 className="ss-header__name" style={{ fontSize: 20 }}>{customer.client?.name || '(sans titre)'}</h2>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                ...
                {sub.code && <span style={{ marginLeft: 4, fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)' }}>{sub.code}</span>}
              </span>
            </div>
          </div>

          <div className="actions-col" style={{ flexShrink: 0 }}>
            <button onClick={onEdit} disabled={editDisabled} className="btn-primary" style={{ fontSize: 12, opacity: editDisabled ? 0.55 : 1, cursor: editDisabled ? 'not-allowed' : 'pointer' }}>
              {editLabel}
            </button>
            {position != null && resiAfter != null && (
              <div className={`status-dot-badge ${resiAfter < 0 ? 'status-dot-badge--danger' : resiAfter < 3 ? 'status-dot-badge--warning' : 'status-dot-badge--success'}`}>
