...
import { getInjectionRigid } from '../../../engines/directionalSubstation.js';
...
function KpiStrip({ label, value, sub, color, alert, sparkValues, onClick, delay = 0 }) {
  ...
  return (
    ...
    >
      ...
      <p className="kpi-value" style={{ color: color || 'var(--text-primary)' }}>
        {value}
      </p>
      ...
      {sub && (
        <p className="kpi-sub" style={{ marginTop: sparkValues ? 3 : 5 }}>
          {sub}
        </p>
      )}
    </div>
    ...
    <div className="card" style={{ overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, minWidth: 700 }}>
          ...
          <tbody>
            {active.map((sub, i) => {
              ...
              const firstReverse = YEARS.find(
                (y) => getInjectionRigid(sub, y, false, projects) < 0,
              );

              return (
                <tr
                  key={sub.id}
                  style={{ background: i % 2 === 0 ? 'var(--bg-raised)' : 'var(--bg-muted)' }}
                >
                  <td
                    style={{
                      padding: '5px 14px',
                  ...
                  {YEARS.map((y) => {
                    ...
                    const invProjects = projects.filter(
                      (p) =>
                        p.year === y &&
                        p.status !== 'annulé' &&
                        (p.effects || []).some((e) => e.ssId === sub.id),
                    );
                    ...
                    return (
                      <td
                        key={y}
                        style={{ padding: '3px', textAlign: 'center', position: 'relative' }}
                        onMouseEnter={(event) =>
                          showTooltip(event, {
                            sub,
                            year: y,
      ...
      >
        {Object.entries(ALERT_CONFIG)
          .filter(([k]) => ['ok', 'caution', 'warning', 'critical'].includes(k))
          .map(([k, c]) => (
            <span
              key={k}
              style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: c.text }}
            >
              <span
                style={{
                  background: c.bg,
...
function SaturationTooltip({ tooltip }) {
  ...
  return (
    ...
    >
      <p style={{ fontWeight: 800, marginBottom: 5, fontFamily: 'var(--font-mono)', fontSize: 11 }}>
        {sub.name} · {year}
      </p>
      <p style={{ color: 'var(--text-muted)', marginBottom: 2 }}>
        Cap. dir. N-1 : <strong style={{ color: 'var(--text-primary)' }}>{f1(cap)} MVA</strong>
      </p>
      <p style={{ color: 'var(--text-muted)', marginBottom: 2 }}>
        Charge rigide : <strong style={{ color: c.text }}>{f1(net)} MVA</strong>
      </p>
      <p
        style={{
          color: res < 0 ? 'var(--red)' : res < 5 ? 'var(--amber)' : 'var(--green)',
          fontWeight: 700,
        }}
      >
...
export function Overview({ substations, onNavigate }) {
  ...
  const warnings = active.filter(
    (s) =>
      YEARS.some(
        (y) => getAlertLevel(getUtilizationWithdrawalRigid(s, y, projects)) === 'warning',
      ) &&
      !YEARS.some(
        (y) => getAlertLevel(getUtilizationWithdrawalRigid(s, y, projects)) === 'critical',
      ),
  );
  ...
  const satByYear = YEARS.map(
    (y) =>
      active.filter(
        (s) => getAlertLevel(getUtilizationWithdrawalRigid(s, y, projects)) === 'critical',
      ).length,
  );
    ...
    <div className="space-y-5 fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
        <KpiStrip
          label="Sous-stations"
          value={active.length}
          sub={`${substations.length} au total`}
          delay={0}
        />
        <KpiStrip
          label="Saturées prél. N-1"
        ...
        />
        <KpiStrip
          label="Inversion de flux"
          value={reverseFlowNow.length}
          alert={reverseFlowNow.length > 0 ? 'orange' : null}
          color={reverseFlowNow.length > 0 ? 'var(--inj)' : 'var(--green)'}
          sub={
            reverseFlowHorizon.length > reverseFlowNow.length
      ...
      {expiredItems.length > 0 && (
        <div className="card" style={{ overflow: 'hidden' }}>
          ...
          >
            <div>
              <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--amber)' }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: 12 }}>
                    {getCustomer(item.req).client?.name || item.req.id}
                  </span>
                  <span style={{ color: 'var(--text-muted)', fontSize: 10, margin: '0 5px' }}>
                    ·
                  </span>
                  <button
                    onClick={() => onNavigate(item.sub.id, 'file')}
                    style={{
                      color: 'var(--accent)',
                      fontSize: 10,
                      fontWeight: 600,
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  ...
                  >
                    {f1(reqClientPrelevTotal(item.req))} MVA
                  </span>
                  <button
                    onClick={() => onNavigate(item.sub.id, 'file')}
            ...
            {criticals.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  style={{
                    background: ALERT_CONFIG.critical.bg,
                    border: `1px solid ${ALERT_CONFIG.critical.border}`,
                    borderRadius: 8,
                    padding: 14,
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, marginTop: 3, color: ALERT_CONFIG.critical.text }}>
                      ...
                      <strong style={{ fontFamily: 'var(--font-mono)' }}>
                        {f1(getResidualWithdrawalRigid(s, sat || 2026, projects))} MVA
                      </strong>
                      {!projects.some(
                        (p) =>
                          p.status !== 'annulé' && (p.effects || []).some((e) => e.ssId === s.id),
                      ) && (
