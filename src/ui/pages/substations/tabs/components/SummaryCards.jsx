...
import {
  ...
  getWithdrawalRigid, getInjectionRigid,
...
function DirectionCard({ isW, view, setView, util, rigid, sat }) {
  ...
  return (
    <div onClick={() => setView(isW ? 'withdrawal' : 'injection')}
      className={`summary-card ${active ? (isW ? 'summary-card--prelev-active' : 'summary-card--inj-active') : ''}`}>
      <div className="summary-card__direction" style={{ color: isW ? 'var(--prelev)' : 'var(--inj)' }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginBottom: 8 }}>
        Util. N-1 : <strong style={{ color: ac.color, fontFamily: 'var(--font-mono)' }}>{pct(util)}</strong>
      </div>
      ...
      <div style={{ fontSize: 10, marginTop: 6, fontWeight: 600,
...
export function SummaryCards({ sub, projects, activeYear, view, setView }) {
  ...
  return (
    <div className="summary-cards-row">
      <DirectionCard isW={true} view={view} setView={setView}
        util={uWR} rigid={wRigid} sat={satW} />
      <DirectionCard isW={false} view={view} setView={setView}
        util={uIR} rigid={Math.abs(iRigid)} sat={satI} />
      {/* Global card */}
      <div className="summary-card summary-card--global" style={{ borderColor: wac.color, background: wac.bg }}>
        <div className="summary-card__direction" style={{ color: wac.color }}>Synthèse globale</div>
        <div style={{ fontSize: 14, fontWeight: 700, color: wac.color, marginBottom: 4 }}>{wac.label}</div>
        <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginBottom: 8 }}>
          Pire sens · {worst === state.worstWithdrawal ? 'prélèvement' : 'injection'}
        </div>
        <div style={{ fontSize: 10, color: wac.text }}>
          {satW && satI ? `Sat. prél. ${satW} · inj. ${satI}`
            : satW ? `Sat. prélèvement ${satW}`
            : satI ? `Sat. injection ${satI}`
