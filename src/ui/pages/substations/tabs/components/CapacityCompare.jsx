...
export function CapacityCompare({ sub, year, view, projects }) {
  ...
  const capN1 = isW ? getDirectCapacityN1AtYear(sub, year, projects) : getReverseCapacityN1AtYear(sub, year, projects);
  const capN = isW ? getDirectCapacityNAtYear(sub, year, projects) : getReverseCapacityNAtYear(sub, year, projects);
  const base = Math.abs(isW ? getWithdrawalBaseNet(sub, year, projects) : getInjectionBaseNet(sub, year, projects));
  const firm = isW ? getWithdrawalFirmReservation(sub, year) : getInjectionFirmReservation(sub, year);
  const flex = isW ? getWithdrawalFlexibleReservation(sub, year) : getInjectionFlexibleReservation(sub, year);
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div className="cap-bar-wrap">
        <div className="cap-bar">
          <div className="cap-bar__track">
            <div className="cap-bar__seg" style={{ width: `${base / maxBar * 100}%`, background: isW ? 'rgba(99,102,241,.55)' : 'rgba(5,150,105,.50)' }} title={`Base: ${f1(base)} MVA`} />
            <div className="cap-bar__seg" style={{ width: `${firm / maxBar * 100}%`, background: isW ? 'rgba(220,38,38,.70)' : 'rgba(5,150,105,.78)' }} title={`Ferme: ${f1(firm)} MVA`} />
            <div className="cap-bar__seg" style={{ width: `${flex / maxBar * 100}%`, background: isW ? 'rgba(217,119,6,.55)' : 'rgba(5,150,105,.38)' }} title={`Flexible: ${f1(flex)} MVA`} />
          </div>
          <div className="cap-bar__marker" style={{ left: `${Math.min(capN1 / maxBar * 100, 99.5)}%`, background: '#1a1230' }}>
            <div className="cap-bar__label" style={{ left: 2 }}>N-1 {f1(capN1)}</div>
          </div>
          {capN && <div className="cap-bar__marker" style={{ left: `${Math.min(capN / maxBar * 100, 99.5)}%`, background: '#9ca3af', width: 2, opacity: .7 }}>
            <div className="cap-bar__label" style={{ left: 2, top: '100%', bottom: 'auto', marginTop: 4 }}>N {f1(capN)}</div>
          </div>}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: 'var(--text-muted)', marginTop: 22, fontFamily: 'var(--font-mono)' }}>
          <span>0</span><span>{f1(maxBar)} MVA</span>
        </div>
      </div>
      ...
      <div className="metric-grid metric-grid--4" style={{ marginTop: 4 }}>
        {[
          ...
          <div key={l} className="metric-box">
            ...
            <div className="metric-box__value" style={{ color: c }}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
        <div className="alert-inline alert-inline--lg" style={{ background: ac.bg, border: `1.5px solid ${ac.border}`, color: ac.text }}>
          <span className="alert-inline__dot" style={{ background: ac.color }} />
          {ac.label} · {pct(rRN1)}
        </div>
        {rRN1 >= 1 && <span style={{ fontSize: 11, color: 'var(--red)', fontWeight: 600 }}>⚠ Saturation prélèvement atteinte</span>}
      </div>
    </div>
  );
}
