...
function SourceTooltip({ req }) {
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-block' }} className="source-tooltip-wrap">
      <span style={{ cursor: 'help', fontSize: 11, opacity: 0.5, marginLeft: 3 }}>ℹ</span>
      <span
      ...
      >
        {prev.map((d, i) => (
          <div key={i}>
            {PREV_USAGE_ICONS[d.type] || '•'} {d.type} : {d.powerMva} MVA{d.flexible ? ' ⚡' : ''}
          </div>
        ))}
        {inj.map((d, i) => (
          <div key={i}>
            {INJ_SOURCE_ICONS[d.source] || '•'} {d.source} : {d.powerMva} MVA
          </div>
        ))}
      </span>
    </span>
...
export function QueueRow({ item, onEdit, onArchive, onDelete, onOpenDossier }) {
  ...
  const grdPF = reqGrdPrelevFerme(req),
    grdPFl = reqGrdPrelevFlexible(req);
  const grdIF = reqGrdInjFerme(req),
    grdIFl = reqGrdInjFlexible(req);

  const accentLeft =
    item.decision === 'acceptable'
      ? 'var(--green)'
      : item.decision === 'liste_attente'
        ? 'var(--red)'
        : item.decision === 'conditionnel'
          ? 'var(--amber)'
          : 'var(--border-strong)';
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '10px 10px' }}>
        <div
          style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-primary)', marginBottom: 3 }}
        >
          {customer.client?.name || '(sans titre)'}
          {item.hasFifoAlert && (
            <span style={{ marginLeft: 5, color: 'var(--amber)', fontSize: 11 }}>⚠</span>
          )}
        </div>
      ...
      <td style={{ textAlign: 'right', padding: '10px 8px' }}>
        {isPrelev && (
          ...
          >
            ↓ {f1(reqClientPrelevTotal(req))}
          </div>
        )}
        {isInj && (
          <div style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--inj)' }}>
            ↑ {f1(reqClientInjTotal(req))}
          </div>
        )}
          ...
          <div style={{ lineHeight: 1.5 }}>
            {grdPF + grdPFl > 0 && (
              ...
              >
                ↓ {f1(grdPF)}
                {grdPFl > 0 && (
                  <span style={{ color: 'var(--amber)', fontWeight: 500 }}> +{f1(grdPFl)}⚡</span>
                )}
              </div>
            )}
            {grdIF + grdIFl > 0 && (
      ...
      <td style={{ textAlign: 'right', paddingRight: 12 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'flex-end' }}>
          ...
          <button className="btn-edit-link" style={{ fontSize: 11 }} onClick={() => onEdit(req)}>
            Modifier
          </button>
          {assessment.status === 'studied' && (
            <button
              onClick={() => onArchive(req, 'raccordée')}
              style={{
                fontSize: 10,
                color: 'var(--inj-text)',
