    ...
    <div style={{ position:'fixed',inset:0,background:'rgba(26,18,48,.55)',backdropFilter:'blur(8px)',
      ...
      <div style={{ background:'var(--bg-raised)',borderRadius:16,boxShadow:'var(--shadow-lg)',
        ...
        <div style={{ padding:'20px 24px',borderBottom:'1px solid var(--border)',
          ...
          <p style={{ fontSize:13,color:'var(--text-secondary)',fontWeight:600 }}>{customer.client?.name}</p>
          {customer.client?.reference && <p style={{ fontSize:11,color:'var(--text-muted)',fontFamily:'var(--font-mono)',marginTop:2 }}>{customer.client.reference}</p>}
        </div>
            ...
            <>
              ...
              {(grdPF + grdFl + grdIF + grdIFl) > 0 && (
                <div style={{ background:'var(--accent-bg)',border:'1px solid var(--border-accent)',borderRadius:10,padding:14 }}>
                  <p style={{ fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:'.07em',color:'var(--accent-muted)',marginBottom:10 }}>Contrat GRD</p>
                  ...
                  {assessment.final?.load?.reason && (
                    <p style={{ fontSize:11,color:'var(--text-secondary)',marginTop:8,fontStyle:'italic' }}>{assessment.final.load.reason}</p>
                  )}
                </div>
              )}

              <div style={{ background:'var(--green-dim)',border:'1.5px solid rgba(5,150,105,.2)',borderRadius:10,padding:14 }}>
                <p style={{ fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:'.07em',color:'var(--green)',marginBottom:10 }}>
                ...
                <p style={{ fontSize:12,color:'var(--text-secondary)',lineHeight:1.55 }}>
                  ...
                  <strong> {retentionMonths} mois</strong>. La durée se modifie ensuite dans l’onglet Raccordés ou dans le dossier.
                </p>
              </div>
            </>
          ) : (
            <p style={{ fontSize:13,color:'var(--text-secondary)' }}>
              La demande sera archivée comme annulée. La capacité réservée sera libérée immédiatement.
            </p>
          )}
