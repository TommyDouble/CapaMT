    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-600">
          <strong className="text-primary">{active.length}</strong> projet(s) réseau impactant cette
          SS ·&nbsp;
          ...
          <tbody>
            {relatedProjects.length > 0 ? (
              relatedProjects.map((proj) => {
                ...
                const myEffects = (proj.effects || []).filter(
                  (e) => e.ssId === sub.id || e.newSS?.id === sub.id,
                );
                const ACTION_SHORT = {
                  modify_tfo: (e) => {
                    ...
                    const addDetails = (e.tfoChanges?.add || []).map(
                      (t) => `${t.id}: ${t.power} MVA (${t.role})`,
                    );
                    const parts = [];
                    ...
                    <td>
                      {myEffects.map((e, i) => (
                        <div key={i} className="text-xs text-gray-600">
                          {ACTION_SHORT[e.action]?.(e) || e.action}
                        </div>
                      ))}
                    </td>
                    <td className="text-right mono text-muted">
                      {proj.cost ? proj.cost.toLocaleString('fr') : '-'}
                    </td>
                    <td className="text-center">
              ...
              <tr>
                <td colSpan={6} className="empty-state">
                  Aucun projet réseau impactant cette sous-station.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {relatedProjects.length === 0 && (
        <p className="text-xs text-muted text-center">
          Pour créer ou modifier des projets réseau, rendez-vous dans l'onglet{' '}
          <strong>Investissements</strong> du menu principal.
        </p>
      )}
    </div>
  );
