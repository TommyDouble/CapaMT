...
export function EvolutionTab({ sub }) {
  ...
  return (
    <div>
      {/* 1. Summary cards */}
      <SummaryCards sub={sub} projects={projects} activeYear={activeYear}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div className="seg-toggle">
          <button className={`seg-toggle__btn${view === 'withdrawal' ? ' active-prelev' : ''}`}
            onClick={() => setView('withdrawal')}>⬆ Prélèvement</button>
          <button className={`seg-toggle__btn${view === 'injection' ? ' active-inj' : ''}`}
            onClick={() => setView('injection')}>⬇ Injection</button>
        </div>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <span className="section-label" style={{ marginRight: 4, marginBottom: 0 }}>Mode</span>
          <button style={modeBtn(mode === 'resultante')} onClick={() => setMode('resultante')}>Résultante</button>
          <button style={modeBtn(mode === 'composantes')} onClick={() => setMode('composantes')}>Composantes</button>
        </div>
      </div>
      ...
      <DirectionalChart sub={sub} projects={projects} view={view} mode={mode}
      ...
      {/* 6. Annual table */}
      <AnnualTable sub={sub} projects={projects} activeYear={activeYear}
