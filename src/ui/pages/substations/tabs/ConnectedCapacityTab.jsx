...
import { computeCapacityImpact, getConnectedRetentionInfo, normalizeConnectedRetentionMonths } from '../../../../engines/capacityImpact.js';
...
function PowerPill({ label, value, color }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
        {label}
      </div>
...
export function ConnectedCapacityTab({ sub, onUpdate, onNavigateToRequest }) {
  const rows = useMemo(() => (sub.connectionRequests || [])
    ...
    .sort((a, b) => String(b.retention.connectedAt || '').localeCompare(String(a.retention.connectedAt || ''))),
  ...
  const expiringSoon = retainedRows.filter(row => row.retention.daysLeft != null && row.retention.daysLeft <= 60);
    ...
    updateRequest(req.id, current => {
      ...
      return {
        ...
        offer: {
          ...
          status: 'offer_connected',
          connectedAt: today,
          connectedRetentionMonths: normalizeConnectedRetentionMonths(offer.connectedRetentionMonths),
        },
      };
    });
  };
    ...
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 8 }}>
        <div className="metric-box">
          ...
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>{retainedRows.length} maintenu(s)</div>
        </div>
        <div className="metric-box" style={{ borderLeft: '3px solid var(--prelev)' }}>
          <div className="metric-box__label">Prél. maintenu</div>
          <div className="metric-box__value" style={{ color: 'var(--prelev)' }}>{f1(retainedLoad)} MVA</div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>encore réservé</div>
        </div>
        <div className="metric-box" style={{ borderLeft: '3px solid var(--inj)' }}>
          <div className="metric-box__label">Inj. maintenue</div>
          <div className="metric-box__value" style={{ color: 'var(--inj)' }}>{f1(retainedInjection)} MVA</div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>encore réservée</div>
        </div>
        <div className="metric-box" style={{ borderLeft: expiringSoon.length ? '3px solid var(--amber)' : '3px solid var(--green)' }}>
          <div className="metric-box__label">Échéance &lt; 60j</div>
          <div className="metric-box__value" style={{ color: expiringSoon.length ? 'var(--amber)' : 'var(--green)' }}>{expiringSoon.length}</div>
        ...
        <div className="metric-box" style={{ borderLeft: '3px solid var(--accent)' }}>
          ...
          <div className="metric-box__value" style={{ color: 'var(--accent)' }}>{releasedRows.length}</div>
      ...
      <div className="card" style={{ overflowX: 'auto', overflowY: 'hidden' }}>
        <div style={{ padding: '11px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Dossiers raccordés</p>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
              Maintien temporaire de capacité après mise en service, sans recalage historique dans l’application.
            </p>
          </div>
        </div>
        ...
        <table style={{ width: '100%', minWidth: 1100, borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'var(--bg-muted)' }}>
              {[
                ...
                <th key={label} style={{ padding: '8px 10px', textAlign: align, fontSize: 9, fontWeight: 700,
                  textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }}>
                  {label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: '38px 20px', color: 'var(--text-muted)', fontSize: 13 }}>
                  Aucun dossier raccordé sur cette sous-station.
                </td>
              </tr>
            )}
            {rows.map(row => {
              const statusColor = row.impact.status === 'CONNECTED_RESERVED'
                ? 'var(--green)'
                : row.retention.missingConnectedAt
                  ? 'var(--amber)'
                  : 'var(--accent)';
              const statusBg = row.impact.status === 'CONNECTED_RESERVED'
                ? 'var(--green-dim)'
                : row.retention.missingConnectedAt
                  ? 'var(--amber-dim)'
                  : 'var(--accent-bg)';
              const statusBorder = row.impact.status === 'CONNECTED_RESERVED'
                ? 'rgba(5,150,105,.25)'
                : row.retention.missingConnectedAt
                  ? 'rgba(217,119,6,.25)'
                  : 'var(--border-accent)';
                ...
                <tr key={row.req.id} className="data-row">
                  <td style={{ padding: '11px 10px', borderBottom: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                      <strong style={{ fontSize: 13, color: 'var(--text-primary)' }}>{row.customer.client?.name || '(sans titre)'}</strong>
                  ...
                  <td style={{ padding: '11px 10px', borderBottom: '1px solid var(--border)', fontSize: 12, color: 'var(--text-secondary)' }}>
                    {row.retention.connectedAt ? fmtShortDate(row.retention.connectedAt) : (
                      <button type="button" className="btn-edit-link" style={{ fontSize: 11, color: 'var(--amber)' }}
                        onClick={() => handleSetToday(row.req)}>
                        Renseigner aujourd’hui
                      </button>
                    )}
                  </td>
                  <td style={{ padding: '11px 10px', borderBottom: '1px solid var(--border)', textAlign: 'center' }}>
                    <input
                    ...
                    />
                    <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 4 }}>mois</span>
                  </td>
                  <td style={{ padding: '11px 10px', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
                    <div style={{ color: row.retention.expired ? 'var(--accent)' : 'var(--text-secondary)', fontWeight: 600 }}>
                  ...
                  <td style={{ padding: '11px 10px', borderBottom: '1px solid var(--border)', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 8px',
                        borderRadius: 20, background: statusBg, color: statusColor, border: `1px solid ${statusBorder}`,
