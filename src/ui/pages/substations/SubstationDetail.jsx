  ...
  return (
    <div
      style={{
        ...
        borderRadius: 8,
        padding: expanded ? '10px 14px 12px' : '8px 14px',
        marginBottom: 12,
        fontSize: 12,
        transition: 'all .15s',
      }}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: 'var(--text-secondary)' }}>
            Ratio inv.{' '}
            <strong className="mono" style={{ color: 'var(--accent)' }}>
              ×{a.reverseCapacityRatio.toFixed(2)}
            </strong>
          </span>
          <span style={{ color: 'var(--text-secondary)' }}>
            Cap. dir. N-1{' '}
          ...
          {a.projectsIncluded.length > 0 && (
            <span style={{ color: 'var(--accent)' }}>
              {a.projectsIncluded.length} projet(s) intégré(s)
            </span>
          )}
        </div>
        <button
        ...
        >
          <span style={{ color: 'var(--text-secondary)' }}>
            ...
            <strong className="mono">
              {f1(wv.maxHistoricLoadBT)} / {f1(wv.maxHistoricLoadMT)} MVA
            </strong>
          </span>
          <span style={{ color: 'var(--text-secondary)' }}>
            Taux BT/MT :{' '}
            <strong className="mono">
              {(wv.growthLoadMaxBT * 100).toFixed(1)} / {(wv.growthLoadMaxMT * 100).toFixed(1)} %/an
            </strong>
          </span>
          {a.projectsIncluded.length > 0 && (
            <span style={{ color: 'var(--text-secondary)' }}>
  ...
  return (
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
      ...
      <div className="dir-card" style={{ flex: '1 1 100px' }}>
        ...
        <div className="dir-card__value" style={{ color: 'var(--text-primary)' }}>
          {f1(capDN1)} <span style={{ fontSize: 11, fontWeight: 500 }}>MVA</span>
        </div>
      ...
      <div className="dir-card" style={{ flex: '1 1 100px' }}>
        ...
        <div className="dir-card__value" style={{ color: 'var(--text-primary)' }}>
          {f1(capRN1)} <span style={{ fontSize: 11, fontWeight: 500 }}>MVA</span>
        </div>
      ...
      <div className="dir-card prelev" style={{ flex: '1 1 120px' }}>
        ...
        <div className="dir-card__value">
          {f1(wRigid)} <span style={{ fontSize: 11, fontWeight: 500 }}>MVA rigide</span>
        </div>
      ...
      <div className="dir-card inj" style={{ flex: '1 1 120px' }}>
        ...
        <div className="dir-card__value">
          {f1(Math.abs(iRigid))} <span style={{ fontSize: 11, fontWeight: 500 }}>MVA rigide</span>
        </div>
        <div className="dir-card__sub">
          {uIR > 0 ? `${(uIR * 100).toFixed(1)}% util. N-1` : '— Pas de contrainte'}
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex items-start justify-between gap-6">
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              {sub.status === 'hors_service' && (
                <span className="ss-badge-offline">Hors service</span>
              )}
            </div>
          ...
          <div className="actions-col">
            <button
              onClick={() => setEditPanel({ mode: 'substationParams', item: sub })}
