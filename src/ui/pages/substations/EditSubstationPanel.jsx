...
const SUB_PAR_DEFAULTS = {
  ...
  foison: Object.fromEntries(
    Object.keys(FOISON_DEFAULTS).map((t) => [t, String(FOISON_DEFAULTS[t])]),
  ),
};
  ...
  return (
    <div className="space-y-2">
      {tfos.map((t, i) => (
        <div
          key={i}
          className="flex items-center gap-2 p-2.5 bg-white rounded-lg border border-light"
        >
...
export function EditSubstationPanel({ item, subName, onSave, onClose }) {
  ...
  const initial = useMemo(() => {
    ...
    return {
      ...
      coeffN: String(tc?.coeffN ?? 0.9),
      coeffN1: String(tc?.coeffN1 ?? 1.0),
      ...
      foison: Object.fromEntries(
        Object.keys(FOISON_DEFAULTS).map((t) => [
          t,
          String(existingFoison[t] ?? FOISON_DEFAULTS[t]),
        ]),
      ),
    };
  }, [item]);
  ...
  const validate = () => {
    const e = {};
    if (form.withdrawalBaseMva === '' || parseFloat(form.withdrawalBaseMva) < 0)
      e.load = 'Valeur ≥ 0 requise';
    ...
    const cN = parseFloat(form.coeffN),
      cN1 = parseFloat(form.coeffN1);
  ...
  const handleSave = () => {
    ...
    const tfos = (form.tfos || [])
      .map((t) => ({ ...t, power: parseFloat(t.power) }))
      .filter((t) => t.power > 0);
    const tc = {
      transformers: tfos,
      coeffN: parseFloat(form.coeffN) || 0.9,
      coeffN1: parseFloat(form.coeffN1) || 1.0,
      reverseCapacityRatio: parseFloat(form.reverseCapacityRatio) || 1.0,
      mtBackup: {
        enabled: !!form.mtBackupEnabled,
        capacity: parseFloat(form.mtBackupCapacity) || 0,
      },
    };
    const foison = Object.fromEntries(
      Object.keys(FOISON_DEFAULTS).map((t) => [
        t,
        parseFloat(form.foison?.[t]) || FOISON_DEFAULTS[t],
      ]),
    );
    ...
    >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <FormRow label="Base prélèvement BT 2025 (MVA)" error={errors.load}>
            <input
              type="number"
              step=".1"
              value={form.withdrawalBaseMva || ''}
              onChange={(e) => set('withdrawalBaseMva', e.target.value)}
          ...
          <FormRow label="Croissance prélèvement BT (%/an)" error={errors.rate}>
            <input
              type="number"
              step=".1"
              value={form.withdrawalGrowthPct || ''}
              onChange={(e) => set('withdrawalGrowthPct', e.target.value)}
              className="input-field"
            />
          </FormRow>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 12 }}>
          <FormRow label="Coeff. N" error={errors.coeffN}>
            <input
              ...
              step=".01"
              value={form.coeffN || ''}
              onChange={(e) => set('coeffN', e.target.value)}
              className="input-field"
            />
          </FormRow>
          <FormRow label="Coeff. N-1" error={errors.coeffN1}>
            <input
              ...
              step=".01"
              value={form.coeffN1 || ''}
              onChange={(e) => set('coeffN1', e.target.value)}
              className="input-field"
            />
          </FormRow>
        </div>
      ...
      <Section title="Secours MT" color="#7c3aed">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          <input
            type="checkbox"
            id="mtb"
            checked={!!form.mtBackupEnabled}
            onChange={(e) => set('mtBackupEnabled', e.target.checked)}
        ...
        {form.mtBackupEnabled && (
          <FormRow label="Capacité MT secours (MVA)">
            <input
              type="number"
              step=".5"
              value={form.mtBackupCapacity || ''}
              onChange={(e) => set('mtBackupCapacity', e.target.value)}
              className="input-field"
              placeholder="Ex: 10"
            />
          </FormRow>
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)' }}>
                  N = {f1(calcCapacityN(tc))} MVA
                </span>
                <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)' }}>
                  N-1 = {f1(calcCapacityN1(tc))} MVA
                </span>
              </div>
            );
          })()}
      ...
      <Section title="Foisonnement" color="#0369a1" defaultOpen={false}>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12, lineHeight: 1.5 }}>
          ...
          <strong> 1.0 = pire cas simultané.</strong> Les valeurs inférieures réduisent la charge
          réservée effective dans les calculs de résiduel.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {Object.keys(FOISON_DEFAULTS).map((type) => (
            ...
            >
              <label
                style={{ fontSize: 12, fontWeight: 600, flex: 1, color: 'var(--text-primary)' }}
              >
      ...
      <Section title="Localisation" color="#0369a1" defaultOpen={false}>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10, lineHeight: 1.5 }}>
          Coordonnées WGS84 pour l'affichage sur la carte réseau. Province de Liège : lat 50.3–50.9
          / lng 5.0–6.2
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <FormRow label="Longitude">
            <input
              ...
              className="input-field"
              placeholder="ex: 5.5720"
            />
          </FormRow>
        </div>
        {form.lat && form.lng && (
          <a
            href={`https://www.openstreetmap.org/?mlat=${form.lat}&mlon=${form.lng}&zoom=15`}
            target="_blank"
      ...
      <Section title="Notes" color="var(--text-muted)">
        <textarea
          ...
          placeholder="Notes libres sur la sous-station…"
        />
      </Section>
    </ModalShell>
  );
